...
public class SwaggerConfig {
    ...
    public Docket productApi() {
    	return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.basePackage("com.stackroute.articleRecommendationService.controller")).paths(PathSelectors.any()).build().apiInfo(apiInfo());
    }
    
    private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Recommendaed API")
				.description("Recommended Services for developers")
